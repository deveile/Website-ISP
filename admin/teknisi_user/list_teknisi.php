...
// Menghitung Notifikasi Pemasangan Menunggu
...
// Menghitung Notifikasi Transaksi Menunggu Verifikasi
...
if (isset($_GET['hapus'])) {
    ...
    // Validasi opsional: pastikan data teknisi ada
    $cek_teknisi = mysqli_query($koneksi, "SELECT id_user FROM tb_teknisi WHERE id_user = $id_hapus");
    
    if (mysqli_num_rows($cek_teknisi) == 0) {
        $pesan_error = 'Data teknisi tidak ditemukan.';
    } else {
        // Hapus dari tb_teknisi terlebih dahulu
        $ok1 = mysqli_query($koneksi, "DELETE FROM tb_teknisi WHERE id_user = $id_hapus");
        // Hapus dari tb_user jika tb_teknisi berhasil dihapus
        $ok2 = $ok1 ? mysqli_query($koneksi, "DELETE FROM tb_user WHERE id_user = $id_hapus AND role = 'teknisi'") : false;

        if ($ok1 && $ok2) {
            $pesan_sukses = 'Akun teknisi berhasil dihapus.';
        } else {
            $pesan_error = 'Gagal menghapus teknisi: ' . mysqli_error($koneksi);
        }
    }
}
...
$result = mysqli_query($koneksi, "
    SELECT u.id_user, t.nama_teknisi, u.username, t.telepon_teknisi
    FROM tb_teknisi t
    JOIN tb_user u ON t.id_user = u.id_user
    ORDER BY t.nama_teknisi ASC
");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Data Teknisi | Anuwani</title>
...
<div class="dashboard-layout">
    ...
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="../admin_user/list_admin.php"><i class="bi bi-person-gear"></i> <span>Kelola Admin</span></a></li>
            <li><a href="index.php" class="active"><i class="bi bi-person-vcard"></i> <span>Kelola Teknisi</span></a></li>
    ...
    <div class="dashboard-content">
        <div class="topbar">
            ...
            <div>
                <h1>Kelola Teknisi</h1>
                <p>Daftar semua akun teknisi lapangan sistem</p>
            </div>
        </div>

        <div class="table-card">
            <div class="table-header">
                <h3>Data Teknisi Lapangan</h3>
            </div>
                ...
                <table class="admin-table">
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <?= $row['telepon_teknisi']
                                    ? htmlspecialchars($row['telepon_teknisi'])
                                    : '<span class="text-muted">- Tidak ada -</span>' ?>
                            </td>
                            <td style="text-align:center;">
                                <button class="btn-hapus"
                                    data-id="<?= (int)$row['id_user'] ?>"
                                    data-nama="<?= htmlspecialchars($row['nama_teknisi'], ENT_QUOTES) ?>"
...
<div class="logout-modal" id="deleteModal">
    <div class="logout-modal-content">
        ...
        <p>Hapus akun teknisi <strong id="namaTeknisi"></strong>?<br>
...
<script>
    ...
    var idHapusTeknisi = 0;

    function openDeleteConfirm(btn) {
        idHapusTeknisi = btn.getAttribute('data-id');
        document.getElementById('namaTeknisi').textContent = btn.getAttribute('data-nama');
    ...
    function jalankanHapus() {
        if (idHapusTeknisi) {
            window.location.href = 'list_teknisi.php?hapus=' + idHapusTeknisi;
        }
    }
